 ...
 */
public class PEdgeView extends PPath
  ...
  /**
   * The FEdge Index that corresponds to the FEdge that
   * we are a view on
   */
  ...
  /**
   * The locator used to find the center point of the node we are connected
   * to
   */
   ...
   */
  public PEdgeView ( int edge_index,
                     PGraphView view ) {
    this ( edge_index,
           view,
           Float.MAX_VALUE,
   ...
   * @param source_end_selected_paint the paint when selected for the source end
   * @param target_end the Tpye of edge end at the target of this edge
   * @param target_end_paint the paint for the target edge end
   * @param target_end_selected_paint the paint when selected for the target end
   */
  public PEdgeView ( int        edge_index,
                     PGraphView view,
                     float      width,
                     ...
                     int        source_end,
                     Paint      source_end_paint,
                     Paint      source_end_selected_paint,
                     int        target_end,
                     Paint      target_end_paint,
    ...
    if ( view == null ) {
      throw new IllegalArgumentException( "A PEdgeView0 must belong to a PGraphView" );
    }
    this.view = view;

    // Set the Index
    if ( edge_index == Integer.MAX_VALUE ) {
      throw new IllegalArgumentException( "A edge_index must be passed to create a PEdgeView0" ); 
    }
    if ( edge_index >= 0 ) {
      this.rootGraphIndex = view.getGraphPerspective().getRootGraphEdgeIndex( edge_index );
    } else {
    ...
    if ( width != Float.MAX_VALUE ) {
      view.setEdgeFloatProperty( rootGraphIndex,
    ...
    //  System.out.println( "Property: Source "+view.getEdgeIntProperty( rootGraphIndex,
    //                                                                      PGraphView.SOURCE_INDEX ) );
    //     System.out.println( "Property: Target "+view.getEdgeIntProperty( rootGraphIndex,
    //                                                                      PGraphView.TARGET_INDEX ) );
    //     System.out.println( "Property: Width "+view.getEdgeFloatProperty( rootGraphIndex,
    //                                                                       PGraphView.EDGE_WIDTH ) );
    //     System.out.println( "Property: Line Type "+view.getEdgeIntProperty( rootGraphIndex,
    //                                                                         PGraphView.EDGE_LINE_TYPE ) );
    //     System.out.println( "Property: Paint "+view.getEdgeObjectProperty( rootGraphIndex,
    //                                                                        PGraphView.EDGE_PAINT ) );
    //     System.out.println( "Property Source FEdge End Type "+view.getEdgeIntProperty( rootGraphIndex,
    //                                                PGraphView.EDGE_SOURCE_END_TYPE ) );
    //     System.out.println( "Property Target FEdge End Type "+view.getEdgeIntProperty( rootGraphIndex,
    //                                                PGraphView.EDGE_TARGET_END_TYPE ) );
  ...
  protected void initializeEdgeView () {
    ...
    source = ( PNodeView )view.getNodeView( view.getEdgeIntProperty( rootGraphIndex,
                                                                    PGraphView.SOURCE_INDEX ) );
    target = ( PNodeView )view.getNodeView( view.getEdgeIntProperty( rootGraphIndex,
                                                                    PGraphView.TARGET_INDEX ) );
    ...
    setSourceEdgeEndType( view.getEdgeIntProperty( rootGraphIndex,
                                                   PGraphView.EDGE_SOURCE_END_TYPE ) );
    sourceEdgeEnd.setPaint( ( Paint )view.getEdgeObjectProperty( rootGraphIndex,
                                                                 PGraphView.EDGE_SOURCE_END_PAINT ) );
    setTargetEdgeEndType( view.getEdgeIntProperty( rootGraphIndex,
                                                   PGraphView.EDGE_TARGET_END_TYPE ) );
    targetEdgeEnd.setPaint( ( Paint )view.getEdgeObjectProperty( rootGraphIndex,
                                                                 PGraphView.EDGE_TARGET_END_PAINT ) );
  ...
  public String toString () {
    ...
    return (source.getNode().getRootGraphIndex() + "->" + target.getNode().getRootGraphIndex() + " (" + rootGraphIndex + ")"); 
    // return ( source.getGraphPerspectiveIndex() +" -> "+target.getGraphPerspectiveIndex() );
  }
  ...
  public void updateConnection ( int node_index ) {
    if ( node_index < 0 ) {
      node_index = view.getGraphPerspective().getRootGraphNodeIndex( node_index );
    }
  ...
  public float getStrokeWidth () {
    return view.getEdgeFloatProperty( rootGraphIndex,
                                      PGraphView.EDGE_WIDTH );
  }
  ...
  public boolean setSelected ( boolean state ) {
    ...
    if ( state != selected ) {
      ...
      if (selected) {
        drawSelected();
        //PNotificationCenter.defaultCenter().postNotification( "EDGE_SELECTION_ADDED_NOTIFICATION", this );
        view.edgeSelected( this );
      } else {
        drawUnselected();
        //        PNotificationCenter.defaultCenter().postNotification( "EDGE_SELECTION_REMOVED_NOTIFICATION", this );
  ...
  public void updateEdgeView () {
    ...
    if ( selfEdge ) {
      ...
      setPathToEllipse( (float) ( sourcePoint.getX() - .25* width ), 
                        (float) ( sourcePoint.getY() - .25* height ),
                        width * (float).5,
                        height * (float).5 ); 
      return;
    }
    ...
    int[] same_edges = view.getGraphPerspective().getAdjacentEdgeIndicesArray( source.getRootGraphIndex(), true, true, true );
    ...
    if ( same_edges.length != 1 ) {
      ...
      // for all EdgeViews.
      // I changed this to eliminate this assumption, instead use the order
      // of the associated indices to rank the edges
      int count = 0;
	...
	//we need to be consistent about what edge we consider to be the
	//source for the purposes of generating the handle, figure this out
	//here
	...
	//figure out the angle between the x-axis and the line running between the nodes. This is used
	//to figure out the angle between the orthongal bisector and the x-axis. This angle is used
	...
	//if we leave this out, the edges can get confused on the straight diagonal
	double y_step = 10.0 * Math.sin(theta);
	double x_step = 10.0 * Math.cos(theta);
	
	//we have to invert the step sometimes to make sure the offset is actually orthogonal
	//to the main line.
	if ( invertX ) {
	  x_step = -x_step;
	} // end of if ()
  ...
  public void updateTargetArrow () {
    targetEdgeEnd.drawIcon(
                           bend.getTargetHandlePoint(),
                           targetPoint);
    //targetEdgeEnd.moveToFront();
    targetPoint.setLocation(
                            targetEdgeEnd.getNewX(),
                            targetEdgeEnd.getNewY());
  }
  ...
  public void updateSourceArrow () {
    sourceEdgeEnd.drawIcon(
                           bend.getSourceHandlePoint(),
                           sourcePoint);
    //sourceEdgeEnd.moveToFront();
    sourcePoint.setLocation(
                            sourceEdgeEnd.getNewX(),
                            sourceEdgeEnd.getNewY());
  }
  ...
  protected void setSourceEdgeEndType ( int type ) {
    ...
    sourcePoint.setLocation( sourceEdgeEnd.getNewX(),
                             sourceEdgeEnd.getNewY() );
  ...
  protected void setTargetEdgeEndType ( int type ) {
    ...
    targetPoint.setLocation( targetEdgeEnd.getNewX(),
                             targetEdgeEnd.getNewY() );
  ...
  protected static void updateEndPointView( Point2D bendEndPoint, int node_shape, Point2D endPoint, PNodeView endNodeView) {
      ...
      case PNodeView.ELLIPSE:
        ...
        if (deltaX == 0) {
          ...
          endPoint.setLocation( endPoint.getX() + nodeWidth*Math.cos( theta ),
                  endPoint.getY() + nodeHeight*Math.sin( theta ));
        }
        break; // case ELLIPSE
      default:
        // polygon edge finding
        final double[] intersection = new double[2];
        final PathIterator border =
          ((PPath) endNodeView).getPathReference().getPathIterator
          (((PNode) endNodeView).getLocalToGlobalTransform(new PAffineTransform()));
        ...
        while (!border.isDone())
        {
          ...
          if (segmentIntersection(intersection,
                                  bendEndPoint.getX(), bendEndPoint.getY(),
   ...
   * intersection point closest to <code>(x1,y1)</code>.<p>
   ...
   */
  public static boolean segmentIntersection(double[] returnVal,
                                            double x1, double y1,
                                            double x2, double y2,
                                            double x3, double y3,
                                            double x4, double y4)
 ...
 ----------------+---+------- yMin             ----------------+---+------ yMin
    ...
    if (y1 > yMax) { x1 = x1 + (x2 - x1) * (yMax - y1) / (y2 - y1); y1 = yMax; }
    if (y3 > yMax) { x3 = x3 + (x4 - x3) * (yMax - y3) / (y4 - y3); y3 = yMax; }
    if (y2 < yMin) { x2 = x1 + (x2 - x1) * (yMin - y1) / (y2 - y1); y2 = yMin; }
    if (y4 < yMin) { x4 = x3 + (x4 - x3) * (yMin - y3) / (y4 - y3); y4 = yMin; }
  ...
  protected void updateTargetPointView() {
    if (!inLargeGraph) {
      ...
      int node_shape = view.getNodeIntProperty(
                            view.getGraphPerspective().getEdgeTargetIndex( rootGraphIndex ),
  ...
  protected void updateSourcePointView() {
    if (!inLargeGraph) {
      ...
      int node_shape = view.getNodeIntProperty(
                            view.getGraphPerspective().getEdgeSourceIndex( rootGraphIndex ),
                            PGraphView.NODE_SHAPE);
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    //System.out.println("FEdge hearing PCS:"+evt.getPropertyName()+": old value:"+ evt.getOldValue()+" new value: "+evt.getNewValue());
    ...
    if (evt.getPropertyName()
      ...
      if (((PNodeView) evt.getNewValue()).getGraphPerspectiveIndex() == target.getGraphPerspectiveIndex()) {
        updateEdgeView();
      } else if (((PNodeView) evt.getNewValue()).getGraphPerspectiveIndex() == source.getGraphPerspectiveIndex()) {
        updateEdgeView();
      }
    } else if (evt.getPropertyName()
               .equals("BoundsChanged") ) {
    ...
    //       //System.out.println( "Incoming Index: "+((PNodeView) evt.getNewValue()).getGraphPerspectiveIndex() );
    //       System.out.println( "Source index: "+source.getGraphPerspectiveIndex() );
    //       System.out.println( "Target Index: "+target.getGraphPerspectiveIndex() );
  ...
  public boolean intersects(Point2D pt) {
    ...
    for (int i = 0; i < (drawPoints.length - 1); i++) {
      Rectangle2D rect = new Rectangle2D.Double(pt.getX() -
                                                (rectSize / 2), pt.getY() - (rectSize / 2), rectSize, rectSize);
