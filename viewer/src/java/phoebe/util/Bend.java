...
public class Bend //implements giny.view.Bend 
{
   ...
   * then the handles aren't displayed and thus this list is of size 0.  
   * Actual handles exist in this list only when the PEdgeView is selected.
   */
  ...
  protected PGraphView view;
	 ...
	 */
	public Bend( Point2D sourcePoint, 
				 Point2D targetPoint, 
				 PEdgeView edgeView ) {
 	...
 	/**
	 * Creates a Bend with handles/handlePoints at the points
	 * specified.
	 * @param sourcePoint The Point2D of the source node.
	 * @param targetPoint The Point2D of the target node.
	 * @param edgeView The associated PEdgeView.
	...
	public Bend( Point2D sourcePoint, 
				 Point2D targetPoint, 
				 PEdgeView edgeView,
				 java.util.List bendPoints ) {

		commonInit(sourcePoint, targetPoint, edgeView);

		for ( int i = 0; i < bendPoints.size(); i++ )
			addHandle( i, (Point2D)(bendPoints.get(i)) );
	}
	 ...
	 */
	private void commonInit ( Point2D sourcePoint, 
				              Point2D targetPoint, 
				              PEdgeView edgeView ) {
	...
	public java.util.List getHandles() {
		...
		for ( int i = 0; i < handlePointList.size(); i++ ) {
			Point2D pt = (Point2D)((Point2D)(handlePointList.get(i))).clone();
			l.add( pt );
		}
		return l; 
	}
  ...
  public void moveHandle_internal(int i, Point2D pt) {
    // System.out.println( "i: "+i+"BEND: "+ handlePointList.size() + "is empty : "+handlePointList.isEmpty() );
    ...
    // System.out.println( "Moving to:"+ pt);
		...
		if ( handleList.size() > 0 ) {
			PHandle h = (PHandle)(handleList.get(i));
  ...
  public Point2D getSourceHandlePoint () {
    ...
    // If there are points, then use the one closest to the sourcePoint. 
      ...
      return (Point2D) handlePointList.get(0);
  }
  ...
  public Point2D getTargetHandlePoint () {
    ...
    // If there are points, then use the one closest to the targetPoint. 
      ...
      return (Point2D) handlePointList.get(handlePointList.size() - 1);
  }
  ...
  public void addHandle ( Point2D pt ) {
    ...
    int index = CubicGenerator.getListIndex(pt, getDrawPoints(), edgeView.getLineType());
   ...
   * Creates the handle at the specified index and its it, as appropriate,
   * to the list.
   *
   * @param insertIndex The index at which to add the PHandle to the 
   * list of handles.
  ...
  protected PNamedHandle createHandle ( Point2D pt ) {
    return new PNamedHandle( pt.toString(), new PPointLocator(edgeView, pt)) {
        public void dragHandle( PDimension dim,
                                PInputEvent aEvent) {
          edgeView.localToParent(dim);
          ((PPointLocator) getLocator()).update( dim.getWidth(),
                                                 dim.getHeight());
          edgeView.updateEdgeView();
        }
      };
  ...
  public void removeHandle ( Point2D pt ) {
    ...
    for (int i = 0; i < handleList.size(); i++) {
      Rectangle2D rect = new Rectangle2D.Double( pt.getX(),
                                                 ...
                                                 1.0,
                                                 1.0);
      if (((PNamedHandle) handleList.get(i)).intersects(rect)) {
        removeHandle(i);
        break;
      }
    }
  ...
  public void removeHandle ( int i ) {
	...
	if ( i >= 0 && i < handleList.size() ) {
        edgeView.removeChild((PNamedHandle) handleList.get(i));
  ...
  public void removeAllHandles () {
    ...
    // drawSelected();
   //  System.out.println( "Bend has: "+handleList.size() +" handles or "+handlePointList.size() );
//     for ( int i = 0; i < handlePointList.size(); ++i ) {
...
//     }
  ...
  public boolean handleAlreadyExists ( Point2D pt ) {
    for (int i = 0; i < handleList.size(); i++) {
      Rectangle2D rect = new Rectangle2D.Double(pt.getX(), pt.getY(), 1.0, 1.0);
      if (((PNamedHandle) handleList.get(i)).intersects(rect))
        return true;
    }
    return false;
	...
	public void drawSelected() {
		...
		for (int i = 0; i < handlePointList.size(); i++) {
			PNamedHandle h = createHandle((Point2D) handlePointList.get(i));
			h.addInputEventListener( edgeView.getEdgeHandler() );
			edgeView.addChild(h);
			handleList.add(h);
		}
	...
	public Point2D[] getDrawPoints() {
		...
		for (int i = 0; i < handlePointList.size(); i++)
			pts[i + 1] = (Point2D) handlePointList.get(i);
