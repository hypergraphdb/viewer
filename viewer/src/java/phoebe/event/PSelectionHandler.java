 ...
 * @author Ben Bederson
 */
public class PSelectionHandler extends PDragSequenceEventHandler
{
	 ...
	 * event handler.
	 */
	public PSelectionHandler(PNode marqueeParent, PNode selectableParent,
			PCamera camera)
	{
	...
	protected void computeMarqueeSelection(PInputEvent pie)
	{
		...
		for (PNode node : selection.keySet())
			if (!allItems.containsKey(node)) unselectList.add(node);
