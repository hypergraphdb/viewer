...
public final class RotationLayouter
{
    ...
    public RotationLayouter(MutablePolyEdgeGraphLayout graph)
    {
        ...
        do
        {
            if(edges.numRemaining() <= 0)
                break;
            int edge = edges.nextInt();
            if(m_graph.isMovableNode(m_graph.edgeSource(edge)) && m_graph.isMovableNode(m_graph.edgeTarget(edge)))
            {
        ...
        } else
        {
            ...
            double hypotenuse = 0.5D * Math.sqrt(rectWidth * rectWidth + rectHeight * rectHeight);
            if(xRectCenter - hypotenuse < 0.0D || xRectCenter + hypotenuse > m_graph.getMaxWidth() || yRectCenter - hypotenuse < 0.0D || yRectCenter + hypotenuse > m_graph.getMaxHeight())
                throw new IllegalStateException("minimum bounding rectangle of movable nodes and edge anchors not free to rotate within MutableGraphLayout boundaries");
    ...
    public void rotateGraph(double radians)
    {
        if(m_translationToOrig == null)
            return;
        AffineTransform3D xform = m_translationToOrig.concatenatePost((new AxisRotation3D((byte)4, radians)).concatenatePost(m_translationFromOrig));
        IntEnumerator nodes = m_graph.nodes();
        do
        {
            if(nodes.numRemaining() <= 0)
