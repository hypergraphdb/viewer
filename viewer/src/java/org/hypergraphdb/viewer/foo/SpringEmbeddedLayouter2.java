 ...
 * <p>
 * Note from Nerius Landys on Wed Oct 6 11:07:24 PDT 2004: The algorithm in this
 * code has been adapted from an existing algorithm in the Cytoscape project. As
 * little as possible has been done to change the algorithmic logic, even where
 * flaws might have been found. The legacy class which this class borrows code
 * from is <code>cytoscape.layout.SpringEmbeddedLayouter</code>.
 * <p>
 ...
 * <p>
 * This class is seriously due for a complete re-write. The algorithm is slow as
 * heck. Please avoid looking at or using this code -- this code is going away
 * in the next Cytoscape release (the one after 2.1).
 */
public final class SpringEmbeddedLayouter2 implements Task
{
	 ...
	 * on a graph. An instance of this class will perform a layout at most once.
	 * The constructor returns quickly; <code>run()</code> does the
	 * computations to perform the layout.
	 * <p>
	 * 
	 * @param graph the graph layout object that this layout algorithm operates
	 * on.
	 */
	...
	/**
	 * @param taskMonitor a hook that a parent application may pass in to get
	 * information regarding what percentage of the layout has been completed
	 * etc., or <code>null</code> to stop this layout algorithm from reporting
	...
	private static class AutoScalingGraphLayout
	{
		...
		void setNodePosition(boolean overrideMovableCheck, int nodeIndex,
				double X, double Y)
		{
			// This is a debugging statement.
			if (alreadyMoved)
		...
		Point2D getNodePosition(int nodeIndex)
		{
			...
			if (o == null)
				return new Point2D.Double(graph
						.getNodePosition(nodeIndex, true), graph
						.getNodePosition(nodeIndex, false));
			else
				return (Point2D) o;
		}
		...
		void moveUnderlyingNodes()
		{
			...
			// We iterate once through just to find min and max bounds for node
			// pos.
			for (int nodeIx = 0; nodeIx < graph.getNumNodes(); nodeIx++)
			{
				Point2D nodePos;
				if ((nodePos = (Point2D) movedNodes.get(new Integer(nodeIx))) == null)
					nodePos = new Point2D.Double(graph.getNodePosition(nodeIx,
							true), graph.getNodePosition(nodeIx, false));
			...
			for (int nodeIx = 0; nodeIx < graph.getNumNodes(); nodeIx++)
			{
				...
				if ((nodePos = (Point2D) movedNodes.get(new Integer(nodeIx))) != null
					...
					graph.setNodePosition(nodeIx, Math.min(Math.max(0, (nodePos
							.getX() - minX)
							* xScaleFactor), graph.getMaxWidth()), Math
							.min(Math.max(0, (nodePos.getY() - minY)
									* yScaleFactor), graph.getMaxHeight()));
			}
			movedNodes = null;
			graph = null;
		}
	...
	private static int[][] calculateNodeDistances(
			MutablePolyEdgeGraphLayout graph)
	{
			...
			while (!(queue.isEmpty()))
			{
				...
				if (index < fromNode)
				{
					...
					for (int i = 0; i < nodes.length; i++)
					{
						...
						distanceThroughToNode = toNodeDistance
								+ distances[index][i];
						if (distanceThroughToNode <= distances[fromNode][i])
						{
							...
							// shorter path
							// calculated from fromNode never will, and is thus
							// complete.
					...
					// distance
					// from tuNode. So now we don't need to put any neighbors on
					// the
					// queue or anything, since they've already been taken care
					// of by
					// the previous calculation.
					continue;
				} // End if toNode has already had all of its distances
					// calculated.
	...
	private PartialDerivatives calculatePartials(PartialDerivatives partials,
			List partialsList, double[] potentialEnergy, boolean reversed,
			final AutoScalingGraphLayout graph)
	{
		...
		while (iterator.hasNext())
		{
			...
			while (true)
			{
				...
				graph.setNodePosition(true, otherNode, graph.getNodePosition(
						otherNode).getX()
						+ (0.001d * (new java.util.Random()).nextDouble()),
						graph.getNodePosition(otherNode).getY()
								+ (0.001d * (new java.util.Random())
										.nextDouble()));
			}
			euclideanDistanceCubed = Math.pow(euclideanDistance, 3);
			distanceFromTouching = euclideanDistance
					- (nodeRadius + otherNodeRadius);
			incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[node][otherNode] * (deltaX - ((m_nodeDistanceSpringRestLengths[node][otherNode] * deltaX) / euclideanDistance))));
			if (!reversed) partials.x += incrementalChange;
			if (otherPartials != null)
			{
				incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[otherNode][node] * (-deltaX - ((m_nodeDistanceSpringRestLengths[otherNode][node] * -deltaX) / euclideanDistance))));
				if (reversed)
			...
			{
				incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (deltaX - (((nodeRadius + otherNodeRadius) * deltaX) / euclideanDistance))));
				if (!reversed) partials.x += incrementalChange;
				if (otherPartials != null)
				{
					incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (-deltaX - (((nodeRadius + otherNodeRadius) * -deltaX) / euclideanDistance))));
			...
			incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[node][otherNode] * (deltaY - ((m_nodeDistanceSpringRestLengths[node][otherNode] * deltaY) / euclideanDistance))));
			if (!reversed) partials.y += incrementalChange;
			if (otherPartials != null)
			{
				incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[otherNode][node] * (-deltaY - ((m_nodeDistanceSpringRestLengths[otherNode][node] * -deltaY) / euclideanDistance))));
				if (reversed)
			...
			if (distanceFromTouching < 0.0)
			{
				incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (deltaY - (((nodeRadius + otherNodeRadius) * deltaY) / euclideanDistance))));
				if (!reversed) partials.y += incrementalChange;
				if (otherPartials != null)
				{
					incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (-deltaY - (((nodeRadius + otherNodeRadius) * -deltaY) / euclideanDistance))));
			...
			incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[node][otherNode] * (1.0 - ((m_nodeDistanceSpringRestLengths[node][otherNode] * (deltaY * deltaY)) / euclideanDistanceCubed))));
			if (reversed)
			{
				if (otherPartials != null)
			...
			if (distanceFromTouching < 0.0)
			{
				incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (1.0 - (((nodeRadius + otherNodeRadius) * (deltaY * deltaY)) / euclideanDistanceCubed))));
				if (reversed)
				{
					if (otherPartials != null)
						otherPartials.xx -= incrementalChange;
			...
			incrementalChange = (m_nodeDistanceSpringScalars[m_layoutPass] * (m_nodeDistanceSpringStrengths[node][otherNode] * (1.0 - ((m_nodeDistanceSpringRestLengths[node][otherNode] * (deltaX * deltaX)) / euclideanDistanceCubed))));
			if (reversed)
			{
				if (otherPartials != null)
			...
			if (distanceFromTouching < 0.0)
			{
				incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (1.0 - (((nodeRadius + otherNodeRadius) * (deltaX * deltaX)) / euclideanDistanceCubed))));
				if (reversed)
				{
					if (otherPartials != null)
						otherPartials.yy -= incrementalChange;
			...
			if (distanceFromTouching < 0.0)
			{
				incrementalChange = (m_anticollisionSpringScalars[m_layoutPass] * (m_anticollisionSpringStrength * (((nodeRadius + otherNodeRadius) * (deltaX * deltaY)) / euclideanDistanceCubed)));
				if (reversed)
				{
					if (otherPartials != null)
						otherPartials.xy -= incrementalChange;
			...
			distanceFromRest = (euclideanDistance - m_nodeDistanceSpringRestLengths[node][otherNode]);
			...
			if (otherPartials != null)
			{
				otherPartials.euclideanDistance = Math
						.sqrt((otherPartials.x * otherPartials.x)
	...
	private PartialDerivatives moveNode(PartialDerivatives partials,
			List partialsList, double[] potentialEnergy,
			AutoScalingGraphLayout graph)
	{
		...
		return calculatePartials(partials, partialsList, potentialEnergy,
				false, graph);
	}
			...
			AutoScalingGraphLayout graph)
	{
		...
		graph
				.setNodePosition(false, node, p.getX() + deltaX, p.getY()
						+ deltaY);
	}
	...
	public void run()
	{
		...
		//for (IntEnumerator it = m_graph.nodes(); it.numRemaining()> 0;) 
		{
			//int i = it.nextInt();
			//System.out.println("SEL2 - node " + i);
			if (m_graph.isMovableNode(i))
			{
				final double deltaX = ((double) rand.nextInt()) / 0x80000000;
				final double deltaY = ((double) rand.nextInt()) / 0x80000000;
		...
		for (int node_i = 0; node_i < m_nodeCount; node_i++)
		{
			for (int node_j = (node_i + 1); node_j < m_nodeCount; node_j++)
			{
				...
				{
					m_nodeDistanceSpringRestLengths[node_i][node_j] = m_nodeDistanceRestLengthConstant
							* nodeDistances[node_i][node_j];
				}
				...
				{
					m_nodeDistanceSpringStrengths[node_i][node_j] = m_nodeDistanceStrengthConstant
		...
		// The second pass takes little time, and the ratio of time for second
		// pass
		...
		for (m_layoutPass = 0; m_layoutPass < m_numLayoutPasses; m_layoutPass++)
		{
			...
			final double percentProgressPerIter = (percentCompletedAfterPasses - percentCompletedBeforePasses)
			...
			for (int iterations_i = 0; (iterations_i < numIterations)
					&& (furthestNodePartials.euclideanDistance >= euclideanDistanceThreshold); iterations_i++)
			{
				if (m_halt) return;
				if (m_layoutPass == 0)
				{
				...
				furthestNodePartials = moveNode(furthestNodePartials,
						partialsList, potentialEnergy, m_autoScaleGraph);
				if (m_layoutPass == 0)
					currentProgress += percentProgressPerIter;
			}
		}
		...
		// The last thing we do is trigger node movement in the underlying
		// graph.
	...
	/**
	 * Signals to a running layout that it's time to abort and exit. This method
	 * is called by a parent application using this layout algorithm. This
	 * method will return immediately when called. <code>run()</code> will
	 * return eventually, and soon after calling <code>halt()</code>.
	 */
	public void halt()
	{
		m_halt = true;
