...
public class RotationScaleLayoutAction extends HGVAction
{

  public RotationScaleLayoutAction()
  {
    super(ActionManager.ROTATE_SCALE_ACTION);
  }
  ...
  public void actionPerformed(ActionEvent e)
  {
    ...
    JDialog dialog = new JDialog(frame, "Rotate/Scale", true);
    ...
    dialog.pack();
    if(frame != null)
      dialog.move((frame.size().width - dialog.size().width) / 3 +
    		frame.location().x,
                (frame.size().height - dialog.size().height) / 8 +
                frame.location().y);
    dialog.setVisible(true); // This blocks until dialog is disposed of.
  }
