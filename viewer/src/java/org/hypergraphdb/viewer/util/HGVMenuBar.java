...
public class HGVMenuBar
  ...
  /**
   * Default constructor.
   * Calls {@link #initialize initializeCytoscapeMenuBar}.
   */
  public HGVMenuBar () {
    initializeMenuBar();
  }
  
  public Map getActionMenuItemMap(){
	  return actionMenuItemMap;
  }
  ...
  public boolean addAction ( Action action ) {
    ...
    if( action instanceof HGVAction ) {
      if( ( ( HGVAction )action ).isInMenuBar() ) {
        menu_name = ( ( HGVAction )action ).getPreferredMenu();
      } else {
        return false;
      }
    } else {
...
//       if( ( in_menu_bar != null ) && ( in_menu_bar.booleanValue() == false ) ) {
//         return false;
//       }
//       menu_name = ( String )map.get( "preferredMenu" );
  ...
  public boolean addAction ( String menu_name, Action action ) {
    ...
    if( actionMenuItemMap != null ) {
      menu_item =
      ( JMenuItem )actionMenuItemMap.get( action );
    }
    if( menu_item != null ) {
      return false;
    }
    ...
    if ( action instanceof HGVAction ) {
      index_object = ( ( HGVAction )action ).getPrefferedIndex();
      if ( ( ( HGVAction )action ).isAccelerated() ) {
        menu_item.setAccelerator(  javax.swing.KeyStroke.getKeyStroke( ( ( HGVAction )action ).getKeyCode(),
                                  ( ( HGVAction )action ).getKeyModifiers() ) );
      }
      ...
      if( index_object != null ) {
        if( index < 0 ) {
          ...
          if( index < 0 ) {
            ...
            Integer effective_last_index = ( Integer )
        ...
        // TODO: REMOVE
        //System.err.println( "Adding action " + action + " to menu " + menu_name + " at index " + index + " since its \"index\" property's value is \"" + index_object + "\"." );
        menu.insert(
          menu_item,
          index
        );
    ...
    if( index_object == null ) {
      ...
      if( menuEffectiveLastIndexMap != null ) {
        ...
        if( effective_last_index != null ) {
          // TODO: REMOVE
          //System.err.println( "Adding action " + action + " to menu " + menu_name + " at the effective_last_index, which is " + effective_last_index + "." );
          menu.insert( menu_item, effective_last_index.intValue() );
          menuEffectiveLastIndexMap.put(
            menu,
      ...
      if( !added_it ) {
        // TODO: REMOVE
        //System.err.println( "Adding action " + action + " to menu " + menu_name + " at the end." );
        menu.add( menu_item );
      }
    }
    if( actionMenuItemMap == null ) {
  ...
  public boolean removeAction ( Action action ) {
    ...
    JMenuItem menu_item = 
      ( JMenuItem )actionMenuItemMap.remove( action );
    if( menu_item == null ) {
      return false;
    }
    String menu_name = null;
...
//       menu_name = ( String )map.get( "preferredMenu" );
   ...
   * multiple menu names, separated by dots ('.').  If any contained menu name
   * does not correspond to an existing menu, then that menu will be created as
   * a child of the menu preceeding the most recent dot or, if there is none,
   * then as a child of this MenuBar.
   */
  ...
  /**
   * CytoscapeMenuBars are unique -- this equals() method returns true
