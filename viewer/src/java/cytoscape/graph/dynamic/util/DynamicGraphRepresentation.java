...
public final class DynamicGraphRepresentation implements java.io.Externalizable {
	...
	private final boolean equalsInternal(final Object o)
	{
	  ...
	  { // Compare m_nodeDepot.
	    FNode currThisNode = m_nodeDepot.m_head.nextNode;
	    FNode currGraphNode = graph.m_nodeDepot.m_head.nextNode;
	  ...
	  { // Compare m_edgeDepot.
	    FEdge currThisEdge = m_edgeDepot.m_head.nextOutEdge;
	    FEdge currGraphEdge = graph.m_edgeDepot.m_head.nextOutEdge;
	  ...
	  { // Compare m_edges.
	    final FEdge[] thisArr = m_edges.m_edgeArr;
	    final FEdge[] graphArr = graph.m_edges.m_edgeArr;
	  ...
	  { // Compare m_nodes.
	    final FNode[] thisArr = m_nodes.m_nodeArr;
	    final FNode[] graphArr = graph.m_nodes.m_nodeArr;
	    if (thisArr.length != graphArr.length) return false;
	    for (int i = 0; i < thisArr.length; i++) {
	      if (thisArr[i] == null) {
	        if (graphArr[i] != null) return false; }
