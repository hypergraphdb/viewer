...
public final class DynamicGraphRepresentation implements java.io.Externalizable
{
	/**
			...
			final boolean undirected)
	{
		...
		return new IntEnumerator() {
			...
			public final int nextInt()
			{
				...
				if (edgeListIndex == 1)
				{
					while (((edge.sourceNode == edge.targetNode) && ((outgoing && edge.directed) || (undirected && !edge.directed)))
			...
			final boolean undirected)
	{
		...
		final DynamicGraphRepresentation graph = this;
		...
		return new IntIterator() {
			...
			private void ensureComputeNext()
			{
				...
				while (theAdj.numRemaining() > 0)
				{
					...
					if (nodeOne == (nodeZero ^ graph.edgeSource(edge) ^ graph
							.edgeTarget(edge)))
					{
						nextEdge = edge;
						return;
			...
			throws java.io.IOException
	{
		...
		for (Node currNode = m_nodeDepot.m_head.nextNode; currNode != null; currNode = currNode.nextNode)
			out.writeInt(currNode.nodeId);
		out.writeInt(-1);
		for (Edge currEdge = m_edgeDepot.m_head.nextOutEdge; currEdge != null; currEdge = currEdge.nextOutEdge)
		...
		{ // m_edges.
			...
			for (int i = 0; i < arrLen; i++)
			{
				...
				out.writeInt((edge.nextOutEdge == null) ? (-1)
						: edge.nextOutEdge.edgeId);
				out.writeInt((edge.prevOutEdge == null) ? (-1)
						: edge.prevOutEdge.edgeId);
				out.writeInt((edge.nextInEdge == null) ? (-1)
						: edge.nextInEdge.edgeId);
				out.writeInt((edge.prevInEdge == null) ? (-1)
						: edge.prevInEdge.edgeId);
			}
		}
		{ // m_nodes.
			...
			for (int i = 0; i < arrLen; i++)
			{
				...
				out.writeInt((node.nextNode == null) ? (-1)
						: node.nextNode.nodeId);
				out.writeInt((node.prevNode == null) ? (-1)
						: node.prevNode.nodeId);
	...
	/*
	 * private final boolean equalsInternal(final Object o) { if (this == o)
	 * return true; if (o == null || !(o instanceof DynamicGraphRepresentation))
	 * return false; final DynamicGraphRepresentation graph =
	 * (DynamicGraphRepresentation) o; if (m_nodeCount != graph.m_nodeCount ||
	 * m_maxNode != graph.m_maxNode || m_edgeCount != graph.m_edgeCount ||
	 * m_maxEdge != graph.m_maxEdge) return false; { // Compare m_nodeDepot.
	 * FNode currThisNode = m_nodeDepot.m_head.nextNode; FNode currGraphNode =
	 * graph.m_nodeDepot.m_head.nextNode; while (currThisNode != null) { if
	 * (currGraphNode == null || currThisNode.nodeId != currGraphNode.nodeId)
	 * return false; currThisNode = currThisNode.nextNode; currGraphNode =
	 * currGraphNode.nextNode; } if (currGraphNode != null) return false; } { //
	 * Compare m_edgeDepot. FEdge currThisEdge = m_edgeDepot.m_head.nextOutEdge;
	 * FEdge currGraphEdge = graph.m_edgeDepot.m_head.nextOutEdge; while
	 * (currThisEdge != null) { if (currGraphEdge == null || currThisEdge.edgeId !=
	 * currGraphEdge.edgeId) return false; currThisEdge =
	 * currThisEdge.nextOutEdge; currGraphEdge = currGraphEdge.nextOutEdge; } if
	 * (currGraphEdge != null) return false; } { // Compare m_edges. final
	 * FEdge[] thisArr = m_edges.m_edgeArr; final FEdge[] graphArr =
	 * graph.m_edges.m_edgeArr; if (thisArr.length != graphArr.length) return
	 * false; for (int i = 0; i < thisArr.length; i++) { if (thisArr[i] == null) {
	 * if (graphArr[i] != null) return false; } else { if (graphArr[i] == null ||
	 * thisArr[i].directed != graphArr[i].directed || thisArr[i].sourceNode !=
	 * graphArr[i].sourceNode || thisArr[i].targetNode !=
	 * graphArr[i].targetNode) return false; if (thisArr[i].nextOutEdge == null) {
	 * if (graphArr[i].nextOutEdge != null) return false; } else { if
	 * (graphArr[i].nextOutEdge == null || thisArr[i].nextOutEdge.edgeId !=
	 * graphArr[i].nextOutEdge.edgeId) return false; } if
	 * (thisArr[i].prevOutEdge == null) { if (graphArr[i].prevOutEdge != null)
	 * return false; } else { if (graphArr[i].prevOutEdge == null ||
	 * thisArr[i].prevOutEdge.edgeId != graphArr[i].prevOutEdge.edgeId) return
	 * false; } if (thisArr[i].nextInEdge == null) { if (graphArr[i].nextInEdge !=
	 * null) return false; } else { if (graphArr[i].nextInEdge == null ||
	 * thisArr[i].nextInEdge.edgeId != graphArr[i].nextInEdge.edgeId) return
	 * false; } if (thisArr[i].prevInEdge == null) { if (graphArr[i].prevInEdge !=
	 * null) return false; } else { if (graphArr[i].prevInEdge == null ||
	 * thisArr[i].prevInEdge.edgeId != graphArr[i].prevInEdge.edgeId) return
	 * false; } } } } { // Compare m_nodes. final FNode[] thisArr =
	 * m_nodes.m_nodeArr; final FNode[] graphArr = graph.m_nodes.m_nodeArr; if
	 * (thisArr.length != graphArr.length) return false; for (int i = 0; i <
	 * thisArr.length; i++) { if (thisArr[i] == null) { if (graphArr[i] != null)
	 * return false; } else { if (graphArr[i] == null || thisArr[i].outDegree !=
	 * graphArr[i].outDegree || thisArr[i].inDegree != graphArr[i].inDegree ||
	 * thisArr[i].undDegree != graphArr[i].undDegree || thisArr[i].selfEdges !=
	 * graphArr[i].selfEdges) return false; if (thisArr[i].nextNode == null) {
	 * if (graphArr[i].nextNode != null) return false; } else { if
	 * (graphArr[i].nextNode == null || thisArr[i].nextNode.nodeId !=
	 * graphArr[i].nextNode.nodeId) return false; } if (thisArr[i].prevNode ==
	 * null) { if (graphArr[i].prevNode != null) return false; } else { if
	 * (graphArr[i].prevNode == null || thisArr[i].prevNode.nodeId !=
	 * graphArr[i].prevNode.nodeId) return false; } if (thisArr[i].firstInEdge ==
	 * null) { if (graphArr[i].firstInEdge != null) return false; } else { if
	 * (graphArr[i].firstInEdge == null || thisArr[i].firstInEdge.edgeId !=
	 * graphArr[i].firstInEdge.edgeId) return false; } if
	 * (thisArr[i].firstOutEdge == null) { if (graphArr[i].firstOutEdge != null)
	 * return false; } else { if (graphArr[i].firstOutEdge == null ||
	 * thisArr[i].firstOutEdge.edgeId != graphArr[i].firstOutEdge.edgeId) return
	 * false; } } } } { // Compare m_firstNode; if (m_firstNode == null) { if
	 * (graph.m_firstNode != null) return false; } else { if (graph.m_firstNode ==
	 * null || m_firstNode.nodeId != graph.m_firstNode.nodeId) return false; } }
	 * return true; }
	 */
}
